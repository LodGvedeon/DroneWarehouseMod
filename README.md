...
Дроны-Фермеры работают по кластерной распределенной системе, стараясь брать задачи равномерно по количеству и местоположению
...
## Оверлей задач (для фермер-дрона)
- **F3** — открыть/закрыть оверлей показывающий текущие запланированные задачи у дронов-фермеров.

Фермеры-дроны берут **семена из сундука склада** и сажают их только **если сезон подходит**.  
Если конкретный вид семян не задан — берут **первый подходящий слот**.  
Мелкий мусор (трава, камни, ветки, сорняки) убирают и складывают в сундук.  
Большие препятствия, деревья, здания и воду игнорируют.
Если в сундуке есть удобрения - применяют их перед посадкой. Приоритет "Удобрение -> Скоророст -> Увлажнитель"
...
- Цветы не собираются (изменяется в конфиге).  
